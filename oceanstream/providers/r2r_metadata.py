...
``R2RFileInfo`` and ``R2RSensorInfo`` extend the generic base types
from :mod:`oceanstream.sensors.processor_base` so they are accepted
anywhere the generic protocol is expected.
"""
...
from oceanstream.sensors.processor_base import FileInfo, SensorInfo


@dataclass
class R2RFileInfo(FileInfo):
...
@dataclass
class R2RSensorInfo(SensorInfo):
...
def parse_bag_info(path: Path) -> R2RSensorInfo:
    ...
    # Extract sensor information — prioritize R2R standard fields
