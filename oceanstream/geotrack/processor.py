...
try:
    from ..sensors.processors.sbe911 import (
...
def _read_single_csv(
    ...
    if not has_coords:
        if not allow_non_spatial:
            ...
            print(
                f"[geotrack] File {filename} has no spatial coordinates, will attempt interpolation"
            )
...
def _convert_nmea_to_csv(
    ...
    try:
        ...
        if verbose:
            print(
                f"[geotrack]   ✓ Converted {stats['lines_parsed']} NMEA sentences → {stats['data_points_written']} CSV rows"
            )
...
def _convert_ctd_to_csv(
    ...
    if not HAS_CTD_SUPPORT:
        raise ImportError(
            "CTD processing requires seabirdscientific. Install with: pip install seabirdscientific"
        )
...
class GeotrackProcessor:
    ...
    def scan_input_source(self, input_source: Path) -> list[Path]:
            ...
            for idx, archive_path in enumerate(archives, 1):
                try:
                    if len(archives) > 1 and self.verbose:
                        print(
                            f"[geotrack] Extracting archive {idx}/{len(archives)}: {archive_path.name}"
                        )
                    ...
                    if archive_ctd_files and not archive_data_files:
                        ...
                        if HAS_CTD_SUPPORT:
                            if self.verbose:
                                print(
                                    f"[geotrack]   CTD archive detected with {len(archive_ctd_files)} hex files"
                                )
                    ...
                    if self.verbose:
                        ...
                        print(
                            f"[geotrack]   ✓ Found {total_files} file(s): {', '.join(file_summary)}"
                        )
        ...
        if ctd_hex_files:
            if not HAS_CTD_SUPPORT:
                self.log(
                    f"found {len(ctd_hex_files)} CTD hex file(s) but seabirdscientific not installed, skipping"
                )
            else:
    ...
    def _process_single_file(self, file_path: Path, data_frames: list[pd.DataFrame]) -> None:
        ...
        try:
            ...
            df_file, file_metadata = _read_single_csv(
                file_path, fname, self.provider, allow_non_spatial=True
            )
        except Exception as e:
            if self.verbose:
        ...
        try:
            ...
            if success:
                if self.verbose:  # pragma: no cover
                    ...
                    non_null_coords = (
                        enriched_df[['latitude', 'longitude']].notna().all(axis=1).sum()
                    )
                    print(
                        f"[geotrack]   ✓ Successfully interpolated coordinates for {non_null_coords}/{len(enriched_df)} rows"
                    )
                return enriched_df
            else:
                if self.verbose:  # pragma: no cover
                    print(
                        f"[geotrack]   ⚠️  Interpolation failed or no reference data available, adding empty coordinates"
                    )
    ...
    def detect_sensors_and_platforms(
        self, df: pd.DataFrame
    ) -> tuple[list[Sensor], list[dict[str, Any]]]:
        ...
        if self.verbose:
            ...
            if platforms:
                ...
                for p in platforms[:3]:  # Show first 3
                    print(
                        f"[geotrack]     • {p.get('id', p.get('platform_id', 'Unknown'))} ({p.get('row_count', '?')} rows)"
                    )
                if len(platforms) > 3:
...
def generate_tiles(
            ...
            if len(meta_df) > 0:
                ...
                if partition_path.exists():
                    sample_df = pd.read_parquet(
                        partition_path,
                        columns=['platform_id']
                        if 'platform_id' in pd.read_parquet(partition_path, nrows=0).columns
                        else None,
                    )
    ...
    if verbose:  # pragma: no cover
        ...
        if include_measurements:
            print(
                f"[tiles] • Measurements: {'auto-selected' if measurement_columns is None else f'{len(measurement_columns)} columns'}"
            )
...
def _group_files_by_campaign_id(
    ...
    for csv_file in csv_files:
        ...
        if not detected_id:
            ...
            if verbose:
                print(
                    f"[geotrack] Warning: Could not detect campaign_id for {csv_file.name}, using filename stem: {detected_id}"
                )
...
def convert(
    ...
    if len(campaign_groups) > 1:
        ...
        for campaign_idx, (detected_campaign_id, campaign_files) in enumerate(
            campaign_groups.items(), 1
        ):
            if verbose:
                print(f"\n{'=' * 70}")
                print(
                    f"[geotrack] Campaign {campaign_idx}/{len(campaign_groups)}: {detected_campaign_id}"
                )
    ...
    if 'campaign_id' not in df.columns or df['campaign_id'].isna().all():
        ...
        print(
            "[geotrack] Please provide --campaign-id parameter or ensure files contain cruise_id metadata."
        )
        return
    ...
    if not dry_run and not force_reprocess:
        ...
        with console.status(
            f"[bold blue]Checking campaign metadata for '{detected_campaign_id}'...", spinner="dots"
        ):
        ...
        if existing_metadata:
            ...
            if output_dir_changed:
                ...
                with console.status(
                    f"[bold blue]Validating previous output location...", spinner="dots"
                ):
                ...
                if old_output_validation['valid']:
                    ...
                    print(
                        f"[geotrack] Use --force-reprocess to clear previous metadata and use the new location."
                    )
                    return
                else:
                    ...
                    if verbose:
                        print(
                            f"[geotrack] Output directory has changed and previous output is invalid:"
                        )
                    ...
                    clear_invalid_campaign_metadata(
                        detected_campaign_id, campaigns_dir, verbose=verbose
                    )
                ...
                if not validation['valid']:
                    if verbose:
                        print(
                            f"[geotrack] Campaign '{detected_campaign_id}' metadata exists but output is invalid:"
                        )
                        for issue in validation['issues']:
                            print(f"  - {issue}")
                        print(
                            f"[geotrack] Clearing invalid campaign metadata to allow reprocessing"
                        )
    ...
    if not (force_reprocess or dry_run):
        ...
        if already_processed:
            ...
            print(
                "[geotrack] Use --force-reprocess to clear metadata and reprocess everything from scratch."
            )
            return
    ...
    if campaign_output_dir.exists() and not force_reprocess:
        ...
        if existing_data is not None and not existing_data.empty:
            ...
            if df_after_dedup < df_before_dedup + len(existing_data):
                if verbose:
                    print(
                        f"[geotrack] Removed duplicates: {df_before_dedup + len(existing_data) - df_after_dedup} rows"
                    )
    ...
    if not dry_run:
        ...
        if verbose:
            print(
                f"[geotrack] Metadata updated: run #{campaign_metadata.get_run_count()}, "
    ...
    if generate_heatmaps and not dry_run:
        ...
        try:
            ...
            if heatmaps_manifest and heatmaps_manifest.get('variables'):
                ...
                if verbose:
                    print(
                        f"[geotrack]   ✓ Generated {len(heatmaps_manifest['variables'])} heatmaps"
                    )
                        ...
                        for hm_file in heatmaps_dir.glob("*"):
                            with hm_file.open('rb') as f:
                                with filesystem.open_output_stream(
                                    f"{cloud_heatmaps_dir}/{hm_file.name}"
                                ) as out:
                                    out.write(f.read())
                        if verbose:
                            print(
                                f"[geotrack]   ✓ Heatmaps uploaded to cloud: {cloud_heatmaps_dir}"
                            )
                    except Exception as e:
                        if verbose:
    ...
    if Settings.SEMANTIC_ENABLE and Settings.SEMANTIC_GENERATE_STAC:
        ...
        processor.emit_stac_metadata(
            ...
            pmtiles_path=pmtiles_path,
            measurement_columns=pmtiles_measurement_columns
            if pmtiles_include_measurements
            else None,
        )
            ...
            if is_cloud and filesystem is not None:
                try:
                    ...
                    with stac_collection_path.open('rb') as f:
                        with filesystem.open_output_stream(
                            f"{cloud_stac_dir}/collection.json"
                        ) as out:
                            out.write(f.read())
                    # Upload items
                    if stac_items_dir.exists():
                        for item_file in stac_items_dir.glob("*.json"):
                            with item_file.open('rb') as f:
                                with filesystem.open_output_stream(
                                    f"{cloud_stac_dir}/items/{item_file.name}"
                                ) as out:
                                    out.write(f.read())
                    if verbose:
    ...
    display_campaign_dir = (
        f"{storage_path.provider}://{cloud_campaign_path}" if is_cloud else campaign_output_dir
    )
    ...
    if heatmaps_generated and heatmaps_manifest:
        print(f"\n▸ COG Heatmaps")
        display_heatmaps_dir = (
            f"{storage_path.provider}://{cloud_campaign_path}/heatmaps"
            if is_cloud
            else heatmaps_dir
        )
        ...
        for var_info in heatmaps_manifest['variables'][:5]:
            var_range = (
                f"({var_info['min']:.2f} - {var_info['max']:.2f} {var_info['unit']})"
                if var_info.get('min') is not None
                else ""
            )
        ...
        print(
            f"  Resolution            : {heatmap_resolution}° ({heatmap_resolution * 111:.1f} km)"
        )
    ...
    if stac_generated:
        print(f"\n▸ STAC Metadata")
        display_stac_dir = (
            f"{storage_path.provider}://{cloud_campaign_path}/stac"
            if is_cloud
            else campaign_output_dir / 'stac'
        )
    ...
    sample_cols = list(df.columns)[:12]
    more_flag = (
        f" (+ {len(df.columns) - len(sample_cols)} more)"
        if len(df.columns) > len(sample_cols)
        else ""
    )
    print(f"\n▸ Column Sample")
