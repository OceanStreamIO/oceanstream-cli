...
WINCH_ARCHIVE = Path(__file__).parent.parent.parent.parent / "raw_data" / "r2r" / "RR2205_151265_winch.tar.gz"
...
@pytest.mark.skipif(
    not WINCH_ARCHIVE.exists(),
    reason=f"Winch archive not available: {WINCH_ARCHIVE}"
)
class TestWinchIntegration:
    ...
    def test_raw_processor_generates_csv(self, extracted_winch_data: Path) -> None:
        ...
        csv_path = winch_raw_processor(
            extracted_winch_data, file_info, sensor_info, descriptor
        )
